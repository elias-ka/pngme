...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        if let Some(pos) = self
            .chunks
            .iter()
            .position(|c| c.typ().to_string() == chunk_type)
        {
            Ok(self.chunks.remove(pos))
        } else {
            Err(Error::msg("chunk type not found"))
        }
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks
            .iter()
            .find(|&c| c.typ().to_string() == chunk_type)
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Error> {
        ...
        if header != Self::STANDARD_HEADER {
            return Err(anyhow!("invalid PNG file signature, got {:?}", header));
        }
        ...
        while i < bytes.len() {
            let b = &bytes[i..];
            let chunk = Chunk::try_from(b)?;
            // 12 is the sum of the sizes of CRC, chunk data length and
            // chunk type fields (4 bytes each)
...
mod tests {
    ...
    fn test_chunk_by_type() {
        ...
        assert_eq!(&chunk.typ().to_string(), "FrSt");
    ...
    fn test_append_chunk() {
        ...
        assert_eq!(&chunk.typ().to_string(), "TeSt");
